...
public class RLBotStateViewer extends PluginPanel
{
    ...
    @Inject
    private RLBotStateViewer(RLBotAgent agent)
    {
        super(false);
        this.agent = agent;
        ...
        actionPanel.add(btnNavTrees);
        actionPanel.add(btnChopTree);
        actionPanel.add(btnNavBank);
        actionPanel.add(btnBankDeposit);
        topPanel.add(actionPanel);

        btnNavTrees.addActionListener(e -> {
            try { agent.triggerNavigateTrees(); } catch (Exception ex) { log.error("Error triggering Navigate Trees", ex); }
        });
        btnChopTree.addActionListener(e -> {
            try { agent.triggerChopTree(); } catch (Exception ex) { log.error("Error triggering Chop Tree", ex); }
        });
        btnNavBank.addActionListener(e -> {
            try { agent.triggerNavigateBank(); } catch (Exception ex) { log.error("Error triggering Navigate Bank", ex); }
        });
        btnBankDeposit.addActionListener(e -> {
            try { agent.triggerBankDeposit(); } catch (Exception ex) { log.error("Error triggering Bank Deposit", ex); }
        });
