...
public class RLBotActionHandler {
    ...
    private void handleCameraRotate(JsonObject data) {
        logger.debug("===== CAMERA ROTATION HANDLER STARTED =====");
        ...
        try {
            ...
            if (data.has("direction") && !data.get("direction").isJsonNull()) {
                ...
                logger.debug("Direction: " + directionStr);
            } else {
            ...
            logger.debug("Using key code: " + keyCode + " (" + KeyEvent.getKeyText(keyCode) + ")");
            
            // Calculate number of key presses
            int numPresses = Math.max(3, Math.min(10, amount / 15));
            logger.debug("Will press key " + numPresses + " times");
            ...
            logger.debug("Canvas obtained: " + canvas);
            
            // DIRECT KEY PRESS APPROACH
            logger.debug("Starting direct key press sequence");
            for (int i = 0; i < numPresses; i++) {
                try {
                    logger.debug("Press #" + (i+1) + " of " + numPresses);
                    ...
                    logger.debug("Dispatching KEY_PRESSED");
                    canvas.dispatchEvent(pressEvent);
                    logger.debug("KEY_PRESSED dispatched");
                    ...
                    logger.debug("Dispatching KEY_RELEASED");
                    canvas.dispatchEvent(releaseEvent);
                    logger.debug("KEY_RELEASED dispatched");
        ...
        logger.debug("===== CAMERA ROTATION HANDLER FINISHED =====");
    }
    ...
    public void handleMouseClick(String button) {
        try {
            ...
            // removed blocking delay between press and release
    ...
    public void handleKeyPress(String keyChar) {
        try {
            ...
            // removed blocking delay between key press and release
    ...
    public void handleTextInput(String text) {
        try {
            ...
            for (char c : text.toCharArray()) {
                ...
                // removed per-character blocking delay
            }
